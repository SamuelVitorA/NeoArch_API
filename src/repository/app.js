...
export async function createOrder(order) {
    try {
        const command = `insert into tb_agendamentos_orders (nome, preco, comeco, fim, telefone, email, telefone_opc, email_opc, imagem) values (?, ?, ?, ?, ?, ?, ?, ?, ?)`; 
        let [response] = await con.query(command, [order.nome, order.preco, order.comeco, order.fim, order.telefone, order.email, order.telefone_opc, order.email_opc, order.imagem]);
